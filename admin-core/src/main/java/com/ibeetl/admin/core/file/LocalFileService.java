...
public class LocalFileService   implements FileService {
	 ...
	 */
	@Override
	public FileItem createImgItem(String name, String bizType, String bizId, Long userId, Long orgId, String fileBatchId,List<FileTag> tags) {
		String fileName = bizId + name.substring(name.lastIndexOf("."));
		...
		coreFile.setFileBatchId(fileBatchId);
		// 图片类别名
		String dir = File.separator + "images" + File.separator + bizType + File.separator + DateUtil.now("yyyyMMdd");
		...
		LocalFileItem item = new LocalFileItem(root);
		item.setName(coreFile.getName());
		item.setPath(coreFile.getPath());
		item.setBizId(coreFile.getBizId());
		item.setBizType(coreFile.getBizType());
		item.setId(coreFile.getId());
		item.setOrgId(coreFile.getOrgId());
		item.setId(coreFile.getId());
		return item;
	}
