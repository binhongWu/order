var Lib = {
    ...
    /* 无分页 初始化搜索 */
    initSearchFormmNoPage : function(form, tableIns, layuiForm) {
        id = form.attr("id");
        Lib.initGenrealForm(form, layuiForm)
        var btn = $("#" + id + "Search")
        btn.on('click', function() {
            Lib.doSearchFormNoPage(form, tableIns, 1);
        });
    },
    /**
	 * 数据求和标签上显示
     ...
     * @param prefixName
     */
	sum : function(form,prefixName){
        console.log('=========== lib sum start =============')
		var url = $("#" + prefixName + "Url").val();
		var sumObj = $("#" + prefixName + "Sum");
        Common.post(url,form.getFormData(true),function(data){
            console.log('lib sum =>',data);
            var text = "&nbsp;";
            for(var key in data){
                text += key + data[key] + "&nbsp;&nbsp;&nbsp;&nbsp;";
        	}
            sumObj.html(text);
            console.log('=========== lib sum end  =============');
        });
	},
	/* form 种各个事件的处理 */
    ...
    /* 无分页搜索 */
    doSearchFormNoPage : function(form, tableIns) {
        var data = form.serializeJson()
        tableIns.reload({
            where : data
        });
