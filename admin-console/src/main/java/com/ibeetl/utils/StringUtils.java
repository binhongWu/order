...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
	...
	public static String getZeroString(String data, int len, String fullStr){
		 String zeroStr = "";
   ...
   public static String formatCardNo(String cardNo){
    	if(StringUtils.isNotBlank(cardNo)){
			...
			if(len>10){
				return cardNo.substring(0, 4)+fillChar('*',5)+cardNo.substring(len-4,len);
			}
		}
		return cardNo;
	}
   public static String formatTelNo(String telNo){
		if(StringUtils.isNotBlank(telNo)){
			...
			if(len == 11){
				return telNo.substring(0, 3)+fillChar('*',4)+telNo.substring(len-4,len);
			}
		}
		return telNo;
	}

	public static String formatIdentityNo(String identityNo){
		if(StringUtils.isNotBlank(identityNo)){
			int len = identityNo.length();
			if(len == 18 || len == 17){
				return identityNo.substring(0, 4)+fillChar('*',10)+identityNo.substring(len-4,len);
			}
		}
		return identityNo;
	}

   public static String fillChar(char char1, int len){
		String result="";
		for(int i=0;i<len;i++){
			result+=char1;
		}
   ...
   public static Map<String,String> splitUrlParam(String sourceStr){
		Map<String,String> map=new HashMap<String,String>();
		String[] sourceStrArray=sourceStr.split("&");
		for(String param:sourceStrArray){
	...
	/**
	 * 生产固定前缀+时间格式+随机数 的流水号
	 * @param prefix
	 * @param num
	 * @param timeFormart
	 * @return
	 */
	public static String serialNumber(String prefix ,int num ,String timeFormart){
		StringBuffer serialnumber = new StringBuffer();
		serialnumber.append(prefix);
		serialnumber.append(DateUtils.nowFormat(timeFormart==null? "yyyyMMddHHmmssSSS":timeFormart));
		if (serialnumber.length()<num){
			serialnumber.append(getRandomNumber(num-serialnumber.length()));
		}
